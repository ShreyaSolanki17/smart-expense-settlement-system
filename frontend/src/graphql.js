...
const CREATE_EXPENSE_MUTATION = `
  mutation CreateExpense($group: ID!, $description: String!, $amount: Decimal!, $paidBy: ID!) {
    createExpense(group: $group, description: $description, amount: $amount, paidBy: $paidBy) {
      expense { id }
    }
  }
`
...
export const graphqlApi = {
  ...
  createExpense: (token, { group, description, amount, paid_by }) =>
    graphqlRequest(token, CREATE_EXPENSE_MUTATION, { group, description, amount, paidBy: paid_by }),
